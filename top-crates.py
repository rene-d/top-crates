...
# assert not semver_match("^0.4.4'", "0.4.14")
# assert semver_match("0.4.*", "0.4.14")
...
n = 0
while len(a.crates) > 0:

    n += 1
    if n > 10000:
        break

    crate, versions = a.crates.popitem()
...
for name, versions in selected_crates.items():
    ...
    if any(e.match(name) for e in a.exclusions):
